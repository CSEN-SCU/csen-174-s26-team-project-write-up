...
**Command to run ALL tests** (PowerShell, from repo root):

```powershell
$env:PYTHONPATH='backend/app-api'; python -m pytest tests webapp/tests backend/app-api/api_unit_test.py -o 'python_files=test_*.py *_test.py *integration.py tdd_*.py'; npm --prefix extension test; npm --prefix backend/coaching-api test
```
